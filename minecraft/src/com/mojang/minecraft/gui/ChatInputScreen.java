...
public final class ChatInputScreen extends GuiScreen {
   ...
   protected final void onKeyPress(char var1, int var2) {
	  if(var1 == 22) {
			...
			int i4 = 32 - this.message.length();
			if(i4 > string3.length()) {
				i4 = string3.length();
			}

			if(i4 > 0) {
		        this.message = this.message + string3.substring(0, i4);
		    }
	  } 
      ...
      if(var2 == 1) {
         ...
         if((var4 = var4.trim()).length() > 0 && this.minecraft.networkManager != null && this.minecraft.networkManager.isConnected()) {
            var3.netHandler.send(PacketType.CHAT_MESSAGE, new Object[]{Integer.valueOf(-1), var4});
         } else if (var4.trim().length() > 0) {
            this.minecraft.hud.addChat(var4);
         }
