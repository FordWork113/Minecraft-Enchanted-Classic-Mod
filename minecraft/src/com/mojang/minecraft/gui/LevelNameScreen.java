...
public final class LevelNameScreen extends GuiScreen {
   ...
   protected final void onButtonClick(Button var1) {
      if(var1.active) {
         if(var1.id == 0 && this.name.trim().length() > 1) {
            ...
            String var6 = this.name.trim() + GuiScreen.getClipboardString();
            int var3 = var10001;
            Minecraft var4 = var10000;
            var10000.levelIo.saveSlot(var4.level, var4.host, var4.session.username, var4.session.sessionId, var2, var3);
   ...
   protected final void onKeyPress(char var1, int var2) {
	   if(var1 == 22) {
			...
			int i4 = 32 - this.name.length();
			if(i4 > string3.length()) {
				i4 = string3.length();
			}

			if(i4 > 0) {
		this.name = this.name + string3.substring(0, i4);
		 }
	  }
