...
public class LoadLevelScreen extends GuiScreen implements Runnable {
   ...
   public void run() {
      try {
         ...
         File levelsfolder = new File(mcDir, "levels");
         File varurl = new File(mcDir, "levels/levels.txt");
         levelsfolder.mkdir();  
         if (!varurl.exists()) {
            BufferedWriter writer = new BufferedWriter(new FileWriter(varurl));
            writer.write("-;-;-;-;-;");
            writer.close();
         }

         BufferedReader varcheck = new BufferedReader(new FileReader(varurl));
         if (!varcheck.readLine().contains(";")) {
            BufferedWriter writer = new BufferedWriter(new FileWriter(varurl));
            writer.write("-;-;-;-;-;");
            writer.close();
         }
   ...
   protected void openLevel(int var1) {
      this.minecraft.loadSlotLevel(this.minecraft.session.username, var1);
