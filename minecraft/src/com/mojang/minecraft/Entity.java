...
import com.mojang.minecraft.level.tile.CobWebBlock;
...
import com.mojang.minecraft.particle.SplashParticle;
...
public abstract class Entity implements Serializable {
   ...
   public void tick() {
      ...
      if (this.isUnderWater()) {
         ...
         if (!this.inWater) {
            ...
            //this.level.playSound("random.splash", this, 0.5F, 1.0F + random.nextFloat() * 0.2F);      
            
            for(int var2 = 0; (double)((float)var2) < 13.0D; ++var2) {
               float var3 = (float)((double)(var1.nextFloat() * 2.0F - 1.0F) * 0.6D);
               float var4 = (float)((double)(var1.nextFloat() * 2.0F - 1.0F) * 0.6D);
               if (this.level.rendererContext$5cd64a7f.settings.particles) {
               /*this.level.particleEngine.spawnParticle(new WaterDropParticle(this.level, this.x + var3, this.y - 0.9F, this.z + var4));
               this.level.particleEngine.spawnParticle(new SplashParticle(this.level, this.x + var3, this.y - 0.9F, this.z + var4));*/
               this.level.particleEngine.spawnParticle(new BubbleParticle(this.level, this.x + var3, this.y - 0.9F, this.z + var4, this.xd, this.yd, this.zd));
               }
            }
         }
      ...
      if (this.isUnderWater()) {
         if (this.counter % 20.0F == 0.0F && this.level.rendererContext$5cd64a7f.settings.particles) {
            this.level.particleEngine.spawnParticle(new BubbleParticle(this.level, this.x, this.y - 0.1F, this.z, 0.0F, -this.yd, 0.0F));
         }
   ...
   public void move(float var1, float var2, float var3) {
      if (this.noPhysics) {
         ...
         float var13;
         float var14;
         if (this.footSize > 0.0F && var18 && this.ySlideOffset < 0.05F && (var6 != var1 || var8 != var3)) {
            var14 = var1;
            var13 = var2;
            float var15 = var3;
            var1 = var6;
            ...
            for(var17 = 0; var17 < var10.size(); ++var17) {
               var1 = ((AABB)var10.get(var17)).clipXCollide(this.bb, var1);
            }
            ...
            for(var17 = 0; var17 < var10.size(); ++var17) {
               var3 = ((AABB)var10.get(var17)).clipZCollide(this.bb, var3);
            }
            ...
            if (var14 * var14 + var15 * var15 >= var1 * var1 + var3 * var3) {
               var1 = var14;
               var2 = var13;
               var3 = var15;
               this.bb = var16.copy();
         ...
         if (this.makeStepSound && !this.minecraft.isFlying) {
            var14 = this.x - var4;
            var13 = this.z - var5;
            this.walkDist = (float)((double)this.walkDist + (double)MathHelper.sqrt(var14 * var14 + var13 * var13) * 0.6D);
            ...
            if (this.walkDist > (float)this.nextStep && var19 > 0) {
               ...
               if (Block.blocks[var19].stepsound != Tile$SoundType.none && (var20 = Block.blocks[var19].stepsound) != Tile$SoundType.glass) {
                  this.playSound("step." + var20.name, var20.getVolume() * 0.75F, var20.getPitch());
                  this.playSound("random." + var20.name, var20.getVolume() * 0.75F, var20.getPitch());
               }
            }
         }
   ...
   public boolean isInCobWeb() {
      int var1 = this.level.getTile((int)this.x, (int)this.y - 1, (int)this.z);
      return Block.blocks[var1] instanceof CobWebBlock;
   }
   ...
   public float getBrightness(float var1) {
      int var2 = (int)this.x;
      int var3 = (int)(this.y + this.heightOffset / 2.0F - 0.5F);
      int var4 = (int)this.z;
      return this.level.getBrightness(var2, var3, var4);
   }
   ...
   public boolean onIce() {
	  Block var5 = Block.blocks[this.level.getTile((int)this.x, (int)(this.y - 1), (int)this.z)];
	  return var5 == Block.ICE;
   }
   
   public boolean onSlime() {
	  Block var5 = Block.blocks[this.level.getTile((int)this.x, (int)(this.y - 1), (int)this.z)];
	  return var5 == Block.SLIME_BLOCK;
   }

   public boolean onMagma() {
      if (this.level.nearProximity(this.bb.grow(0.1F, 0.0F, 0.0F), Block.MAGMA.id)) {
         return true;
      } else if (this.level.nearProximity(this.bb.grow(0.0F, 0.0F, 0.1F), Block.MAGMA.id)) {
         return true;
      } else {
         return this.level.nearProximity(this.bb.grow(0.0F, 0.1F, 0.0F), Block.MAGMA.id);
      }
   }
   
   public boolean onCactus() {
	      if (this.level.nearProximity(this.bb.grow(0.1F, 0.0F, 0.0F), Block.CACTUS.id)) {
	         return true;
	      } else if (this.level.nearProximity(this.bb.grow(0.0F, 0.0F, 0.1F), Block.CACTUS.id)) {
	         return true;
	      } else {
	         return this.level.nearProximity(this.bb.grow(0.0F, 0.1F, 0.0F), Block.CACTUS.id);
	      }
	   }
