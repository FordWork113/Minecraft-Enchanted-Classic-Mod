...
public final class SandBlock extends Block {
   ...
   public final void onAdded(Level level, int x, int y, int z) {
	      for(int var7 = x - 1; var7 <= x + 1; ++var7) {
	         for(int var6 = z - 1; var6 <= z + 1; ++var6) {
	            if (level.isLava(var7, y, var6) && this == Block.SAND && !level.rendererContext$5cd64a7f.gamemode.isCreative()) {
	               level.setTile(x, y, z, Block.GLASS.id);
	            } else if (level.isLava(var7, y, var6) && this == Block.CLAY && !level.rendererContext$5cd64a7f.gamemode.isCreative()) {
		           level.setTile(x, y, z, Block.BRICK.id);
		        }
	         }
	      }
