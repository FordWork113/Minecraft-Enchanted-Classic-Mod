...
public class LiquidBlock extends Block {
   ...
   public final int getTextureId(int texture) {
	  return this.type == LiquidType.LAVA ?this.textureId : (texture == 1?this.textureId : (texture == 0 ? this.textureId : this.textureId + 32));
   }
   ...
   public void update(Level level, int x, int y, int z, Random rand) {
      ...
      if (this.type == LiquidType.LAVA && level.getTile(x, y + 1, z) == 0 && !level.isSolidTile(x, y + 1, z)) {
          float f = (float)x + rand.nextFloat();
          float f2 = (float)y + this.y2;
          float f3 = (float)z + rand.nextFloat();
          level.particleEngine.spawnParticle(new LavaDropParticle(level, f, f2, f3));
       }

   }

   public final void randomTick(Level level, int x, int y, int z, Random rand) {  
	  if (this.type == LiquidType.LAVA && level.getTile(x, y + 1, z) == 0 && !level.isSolidTile(x, y + 1, z)) {
	    float f = (float)x + random.nextFloat();
	    float f2 = (float)y + this.y2;
	    float f3 = (float)z + random.nextFloat();
	    level.particleEngine.spawnParticle(new LavaDropParticle(level, f, f2, f3));
	  }
   }

   private boolean canFlow(Level var1, int var2, int var3, int var4) {
      if(this.type == LiquidType.WATER || this.type == LiquidType.LAVA) {
         for(int var7 = var2 - 2; var7 <= var2 + 2; ++var7) {
            for(int var5 = var3 - 2; var5 <= var3 + 2; ++var5) {
               for(int var6 = var4 - 2; var6 <= var4 + 2; ++var6) {
                  if(var1.getTile(var7, var5, var6) == Block.SPONGE.id && this.type == LiquidType.WATER) {
                     return false;
                  }

                  if(var1.getTile(var7, var5, var6) == Block.LAVA_SPONGE.id && this.type == LiquidType.LAVA) {
                     return false;
                  }
               }
            }
   ...
   public void onNeighborChange(Level var1, int var2, int var3, int var4, int var5) {
      if(var5 != 0) {
         ...
         if(var6 == LiquidType.WATER && this.type == LiquidType.LAVA) {
         	 var1.playSound("random.fizz", (float)var2 + 0.5F, (float)var3 + 0.5F, (float)var4 + 0.5F, 0.5F, 2.6F + (this.random.nextFloat() - this.random.nextFloat()) * 0.8F);
             var1.setTile(var2, var3, var4, Block.MAGMA.id);
             return;
         }
