...
public final class Vec3D {
   ...
   public float y;
   public float z;
   private static List<Vec3D> pool = new ArrayList();
   ...
   public final Vec3D set(float var1, float var3, float var5) {
	      this.x = var1;
	      this.y = var3;
	      this.z = var5;
	      return this;
   ...
   public static Vec3D newPermanent(float var0, float var2, float var4) {
      return new Vec3D(var0, var2, var4);
   }

   
   public static Vec3D newTemp(float var0, float var2, float var4) {
	      if (poolPointer >= pool.size()) {
	         pool.add(newPermanent(0.0F, 0.0F, 0.0F));
	      }

	      return ((Vec3D)pool.get(poolPointer++)).set(var0, var2, var4);
	   }
